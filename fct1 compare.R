...
#Extract cases:
toMatch <- c("ILI CASES", "ILI OUTPATIENTS", "SARI CASES", "SARI DEATHS", "SARI INPATIENTS")#All usable
...
fluBar <- function(dataFull, cases){
  ...
  names(countryYear)[1:4] <- paste(c("country", "year", "case", "value"))
  #print(countryYear)
  ...
  toplot <- array(0,c(ly,numC*lc))#Columns are time series
  for (i in 1:numC){
