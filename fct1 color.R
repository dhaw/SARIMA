...
#FLUID
#Load all data:
listcsv <- dir(pattern = "DynamicFLUID*")
...
fluBar <- function(dataFull, cases){
  ...
  for (i in 1:(numC-2)){
    ...
    barplot(t(as.matrix(toplot[Y1:numY, i, ])), ylab=nameC[i], lwd=10, lend="square", xaxt="n", col=terrain.colors(6))
  }
  ...
  barplot(t(as.matrix(toplot[Y1:numY, (numC-1), ])), ylab=nameC[numC-1], lwd=10, lend="square", col=terrain.colors(6))
  barplot(t(as.matrix(toplot[Y1:numY, numC, ])), ylab=nameC[numC], lwd=10, lend="square", col=terrain.colors(6))
  #}
  ...
  for (i in 1:numC){
    ...
    if (numA>0){
      ...
      for (j in 1:(numA-1)){
        ...
        barplot(t(as.matrix(toplot[Y1:numY, j, ])), ylab=nameA[i], lwd=10, lend="square", xaxt="n", col=terrain.colors(6))
      }
      #barplot(t(as.matrix(toplot[Y1:numY,((numA-1)*lc+1):(numA*lc)])), ylab=nameA[numA], lwd=10, lend="square")
      barplot(t(as.matrix(toplot[Y1:numY, numA, ])), ylab=nameA[numA], lwd=10, lend="square", col=terrain.colors(6))
